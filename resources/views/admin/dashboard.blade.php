                ...
                <div class="md:hidden space-y-4">
                    ...
                    <div class="bg-gradient-to-r from-gray-50 to-white border border-gray-200 rounded-xl p-5 space-y-4 hover:shadow-lg transition-all duration-200 hover:border-blue-200">
                        <div class="flex justify-between items-start">
                            <div class="space-y-1">
                                <h4 class="font-bold text-gray-800 text-lg">{{ $payment->full_name }}</h4>
                            ...
                            <span class="px-3 py-2 rounded-xl text-xs font-bold shadow-sm
                                @if($payment->status == 'confirmed') bg-green-100 text-green-800 border border-green-200
                                @elseif($payment->status == 'cancelled') bg-red-100 text-red-800 border border-red-200
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1">
                                ...
                                <p class="font-semibold text-gray-800">{{ $payment->created_at->format('d M Y') }}</p>
                            </div>
                            <div class="space-y-1">
                                <span class="text-xs text-gray-500 font-medium uppercase tracking-wide">Amount</span>
                                <p class="font-bold text-green-600 text-lg">Rp {{ number_format($payment->total_amount, 0, ',', '.') }}</p>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="bg-gray-100 rounded-lg p-3 border-l-4 border-blue-500">
                                <span class="font-mono text-sm text-gray-800">{{ $payment->payment_code }}</span>
                            </div>
                        </div>
                        
                        <form action="{{ route('payments.update.status', $payment->id) }}" method="POST" class="pt-2">
                            @csrf
                            @method('PUT')
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-gradient-to-r hover:from-blue-25 hover:to-blue-50 transition-all duration-200 group">
                                    ...
                                    <td class="p-4 text-gray-600 font-medium">{{ $payment->created_at->format('d M Y') }}</td>
                                    <td class="p-4">
                                    ...
                                    <td class="p-4 font-bold text-green-600 text-lg">Rp {{ number_format($payment->total_amount, 0, ',', '.') }}</td>
                                    <td class="p-4">
                                        <div class="bg-gray-50 rounded-lg p-2 border-l-4 border-blue-500">
                                            <span class="font-mono text-sm text-gray-700">{{ $payment->payment_code }}</span>
                                        </div>
                                    </td>
                                    <td class="p-4">
                                    ...
                                    <td class="p-4">
                                        <form action="{{ route('payments.update.status', $payment->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PUT')
                                            <select name="status" onchange="this.form.submit()" 
