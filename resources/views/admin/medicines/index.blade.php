            ...
            <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                ...
                <div class="grid grid-cols-3 gap-3 sm:gap-4 w-full lg:w-auto">
                    <div class="bg-green-50 border border-green-200 rounded-lg p-3 text-center">
                        <div class="text-lg sm:text-2xl font-bold text-green-700">{{ $medicines->count() }}</div>
                        <div class="text-xs sm:text-sm text-green-600">Total</div>
                    </div>
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-center">
                        <div class="text-lg sm:text-2xl font-bold text-yellow-700">{{ $medicines->where('stok', '<=', 10)->count() }}</div>
                        <div class="text-xs sm:text-sm text-yellow-600">Low Stock</div>
                    </div>
                    <div class="bg-red-50 border border-red-200 rounded-lg p-3 text-center">
                        <div class="text-lg sm:text-2xl font-bold text-red-700">{{ $medicines->filter(function($m) { return $m->tanggal_kadaluarsa->isPast(); })->count() }}</div>
                            ...
                            <div class="flex items-center">
                                @if($medicine->image)
                                    <img class="h-12 w-12 rounded-lg object-cover" 
                                         src="{{ asset($medicine->image) }}" 
                                         alt="{{ $medicine->nama }}"
                                         onclick="showImageModal('{{ asset($medicine->image) }}', '{{ $medicine->nama }}')">
                                @else
                                ...
                                @endif
                                <div class="ml-3">
                                    <div class="font-medium text-gray-900">{{ $medicine->nama }}</div>
                                    <div class="text-sm text-gray-500">{{ $medicine->formatted_price }}</div>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <a href="{{ route('admin.medicines.show', $medicine->id_medicine) }}"
                                ...
                                <a href="{{ route('admin.medicines.edit', $medicine->id_medicine) }}"
                                   class="p-2 text-green-600 hover:bg-green-50 rounded-lg">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <button onclick="showDeleteModal('{{ $medicine->id_medicine }}', '{{ addslashes($medicine->nama) }}')"
                        ...
                        <div class="grid grid-cols-2 gap-3 text-sm">
                            <div>
                                ...
                                <span class="ml-1 px-2 py-1 text-xs rounded-full 
                                    {{ $medicine->kategori == 'Antibiotik' ? 'bg-red-100 text-red-800' : 
                            ...
                            <div>
                                ...
                                <span class="ml-1 font-medium">{{ $medicine->stok }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Expiry:</span>
                                <span class="ml-1">{{ $medicine->tanggal_kadaluarsa->format('d/m/Y') }}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">Producer:</span>
                                <span class="ml-1">{{ $medicine->produsen }}</span>
                            </div>
                        </div>
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        @endif
                                        <div class="ml-3">
                                            <div class="text-sm font-medium text-gray-900">{{ $medicine->nama }}</div>
                                            <div class="text-sm text-gray-500">ID: {{ $medicine->id_medicine }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm font-medium text-green-600">{{ $medicine->formatted_price }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $medicine->stok }} units</div>
                                    <span class="px-2 py-1 text-xs font-medium rounded-full 
                                        @if($medicine->stock_status == 'Habis') bg-red-100 text-red-800
                                        @elseif($medicine->stock_status == 'Stok Menipis') bg-yellow-100 text-yellow-800
                                        @else bg-green-100 text-green-800 @endif">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $medicine->tanggal_kadaluarsa->format('d/m/Y') }}</div>
                                    <span class="px-2 py-1 text-xs font-medium rounded-full 
                                        @if($medicine->expiry_status == 'Kadaluarsa') bg-red-100 text-red-800
                                        @elseif($medicine->expiry_status == 'Akan Kadaluarsa') bg-yellow-100 text-yellow-800
                                        @else bg-green-100 text-green-800 @endif">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-900">{{ $medicine->produsen }}</td>
                                <td class="px-6 py-4">
