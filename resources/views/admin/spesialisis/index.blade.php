...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold">{{ $spesialisis->count() }}</p>
                    </div>
            ...
            @foreach($topSpecializations as $specialization => $specialists)
            <div class="bg-white rounded-xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-shadow duration-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-gray-600 text-sm font-medium truncate" title="{{ $specialization }}">
                            {{ Str::limit($specialization, 15) }}
                        </p>
                        <p class="text-2xl font-bold text-gray-800">{{ $specialists->count() }}</p>
                        <p class="text-xs text-gray-500 mt-1">{{ $specialists->count() == 1 ? 'Specialist' : 'Specialists' }}</p>
                    </div>
            ...
            @for($i = $topSpecializations->count(); $i < 3; $i++)
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-200 p-6 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($specializations->sortByDesc(function($items) { return $items->count(); }) as $specialization => $specialists)
                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors duration-150">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-medium text-gray-700 truncate" title="{{ $specialization }}">
        ...
        <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl border border-gray-200">
            <div class="p-6 text-gray-900">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-4 mt-2">
                            <span class="text-sm text-gray-500">
                                Total: <span class="font-semibold text-gray-700">{{ $spesialisis->count() }}</span> specialists
                            </span>
                            <span class="text-sm text-gray-500">
                                Specializations: <span class="font-semibold text-gray-700">{{ $specializations->count() }}</span> types
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        ...
                        <div class="sm:w-64">
                            ...
                            <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                ...
                                @foreach($specializations->sortBy(function($items, $key) { return $key; }) as $specialization => $specialists)
                                <option value="{{ $specialization }}">{{ $specialization }} ({{ $specialists->count() }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                                ...
                                <tr class="hover:bg-gray-50 transition-colors duration-150 specialist-row">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900 specialist-name">{{ $nama }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900 max-w-xs truncate specialist-address" title="{{ is_array($spesialis->alamat) ? implode(', ', $spesialis->alamat) : $spesialis->alamat }}">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900 max-w-xs truncate specialist-duty" title="{{ is_array($spesialis->tempatTugas) ? implode(', ', $spesialis->tempatTugas) : $spesialis->tempatTugas }}">
                ...
                <div class="mt-4 flex items-center justify-between text-sm text-gray-600">
                    <div id="resultsInfo">
                        Showing {{ $spesialisis->count() }} of {{ $spesialisis->count() }} specialists
                    </div>
